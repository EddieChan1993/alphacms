...
class PHPExcel_Writer_Excel5_Workbook extends PHPExcel_Writer_Excel5_BIFFwriter
{
	...
	public function addXfWriter($style, $isStyleXf = false)
	{
		...
		// Add the number format if it is not a built-in core and not already added
	...
	private function _writeWindow1()
	{
		...
		// not supported by PHPExcel, so there is only core selected sheet, the active
	...
	private function _writeSharedStringsTable()
	{
		...
		foreach (array_keys($this->_str_table) as $string) {
			...
			while ($finished === false) {
				...
				// normally, there will be only core cycle, but if string cannot immediately be written as is
				// there will be need for more than core cylcle, if string longer than core record data block, there
				...
				if (strlen($recordData) + strlen($string) <= $continue_limit) {
					...
					if (strlen($recordData) + strlen($string) == $continue_limit) {
						// we close the record data block, and initialize a new core
					...
					// If the string is very long it may need to be written in more than core CONTINUE record.
					...
					// 2. space remaining is greater than or equal to minimum space needed
						...
						// core block fininshed, store the block data
		...
		// combine into core chunk with all the blocks SST, CONTINUE,...
		$chunk = '';
