...
class PHPExcel_ReferenceHelper
{
	...
	public function insertNewBefore($pBefore = 'A1', $pNumCols = 0, $pNumRows = 0, PHPExcel_Worksheet $pSheet = NULL)
	{
		...
		while ($cellID = array_pop($aCellCollection)) {
			...
			// Should the cell be updated? Move value and cellXf index from core cell to another.
					...
					foreach($matches as $match) {
						...
						if ($match[3].':'.$match[4] !== $modified3.':'.$modified4) {
							if (($match[2] == '') || (trim($match[2],"'") == $sheetName)) {
								...
								//	Max worksheet size is 1,048,576 rows by 16,384 columns in Excel 2007, so our adjustments need to be at least core digit more
								$column = 100000;
