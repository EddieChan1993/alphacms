...
class PHPUnit_Framework_TestSuite implements PHPUnit_Framework_Test, PHPUnit_Framework_SelfDescribing, IteratorAggregate
{
        ...
        if ($constructor !== null) {
            ...
            if (count($parameters) < 2) {
                ...
                try {
                    ...
                    $message = sprintf(
                        'TestService for %s::%s marked incomplete by data provider',
                        $className,
                        $name
                    );
                    ...
                    $message = sprintf(
                        'TestService for %s::%s skipped by data provider',
                        $className,
                        $name
                    );
                ...
                // TestService method with @dataProvider.
                if (isset($data)) {
    ...
    protected function addTestMethod(ReflectionClass $class, ReflectionMethod $method)
    {
        ...
        if (!$method->isPublic()) {
            $this->addTest(
                self::warning(
                    sprintf(
                        'TestService method "%s" in test class "%s" is not public.',
                        $name,
                        $class->getName()
                    )
                )
    ...
    {
        if (strpos($method->name, 'TestService') === 0) {
            return true;
        }
