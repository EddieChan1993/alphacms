...
class PHPUnit_TextUI_Command
{
    ...
    public function run(array $argv, $exit = true)
    {
        ...
        if (is_object($this->arguments['TestService']) &&
            $this->arguments['TestService'] instanceof PHPUnit_Framework_Test) {
            $suite = $this->arguments['TestService'];
        } else {
            $suite = $runner->getTest(
                $this->arguments['TestService'],
        ...
        unset($this->arguments['TestService']);
    ...
    protected function handleArguments(array $argv)
    {
        ...
        if (!isset($this->arguments['TestService'])) {
            if (isset($this->options[1][0])) {
                $this->arguments['TestService'] = $this->options[1][0];
            }
            ...
            if (isset($this->arguments['TestService']) &&
                is_file($this->arguments['TestService']) &&
                substr($this->arguments['TestService'], -5, 5) != '.phpt') {
                $this->arguments['testFile'] = realpath($this->arguments['TestService']);
                $this->arguments['TestService']     = substr($this->arguments['TestService'], 0, strrpos($this->arguments['TestService'], '.'));
            }
        }

        if (!isset($this->arguments['testSuffixes'])) {
            $this->arguments['testSuffixes'] = array('TestService.phpice.php', '.phpt');
        }
        ...
        if (isset($this->arguments['configuration'])) {
            ...
            if (!isset($this->arguments['TestService'])) {
                ...
                if ($testSuite !== null) {
                    $this->arguments['TestService'] = $testSuite;
                }
            }
        ...
        if (isset($this->arguments['TestService']) && is_string($this->arguments['TestService']) && substr($this->arguments['TestService'], -5, 5) == '.phpt') {
            $test = new PHPUnit_Extensions_PhptTestCase($this->arguments['TestService']);

            $this->arguments['TestService'] = new PHPUnit_Framework_TestSuite;
            $this->arguments['TestService']->addTest($test);
        }
...
TestService Selection Options:
  ...
  --test-suffix ...         Only search for test in files with specified
                            suffix(es). Default: TestServiceService.php,.phpt

TestService Execution Options:
